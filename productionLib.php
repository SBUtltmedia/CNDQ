<?php
/**
 * Shared production logic library
 */

function calculateProduction(&$userData) {
    ...
    // If no production possible, return early
    if ($d == 0 && $s == 0) {
        return false; 
    }
    ...
    // We append to a production log if needed, or just update lastProduction
    // Let's accumulate "lastProduction" for the user to see "recent activity"
    // OR just overwrite "lastProduction" to show what just happened.
    // Given the continuous nature, maybe we want to see "Total Produced"?
    // For now, let's update lastProduction so the UI shows "Just produced X!"
    ...
    return true; // Production occurred
}
