  ...
  try {
    ...
    for (const url of urlsToTry) {
      try {
        ...
        // Extract the base URL from the login URL before clicking
        const urlObj = new URL(url);
        workingBaseUrl = `${urlObj.protocol}//${urlObj.host}`;

        await page.click('a[href="?user=test_mail1@stonybrook.edu"]'); // Login as Team 1
        console.log('✓ Selected Team 1 user');
        success = true; // Set success to true before waiting for navigation
    ...
    // Use the same base URL that worked for login
    const adminUrl = `${workingBaseUrl}/admin`;

    success = false;
    try {
      console.log(`Navigating to admin page: ${adminUrl}`);
      await page.goto(adminUrl, { waitUntil: 'networkidle2' });
      console.log(`✓ Reached admin page at ${adminUrl}`);
      await page.waitForSelector('#npc-list');
      success = true;
    } catch (error) {
      console.log(`✗ Failed to reach admin page at ${adminUrl}: ${error.message}`);
    }
